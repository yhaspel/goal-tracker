...
The repository contains the whole first-release application apart from Stage 7: the React interface for every account and board workflow, the account API, credential rotation and the operator rescue runbook, the board API, and English, Hebrew, and Russian dictionaries. Schema migrations run through version 4. There are **no real production users or data**, and production still serves the Stage 1 placeholder. Read the [Stage 1 feasibility report](docs/stage-1-feasibility.md) and the [Stage 2](docs/stage-2-completion.md), [Stage 4](docs/stage-4-completion.md), and [Stage 5](docs/stage-5-completion.md) completion reports before changing authentication, the board, or the interface.

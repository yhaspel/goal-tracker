...
- Start at the [development-plan index](development-plans/README.md). Implement the numbered stages in order. The open work is the two checks in [`handoff-remaining-checks.md`](development-plans/handoff-remaining-checks.md), which close [Stage 3](development-plans/stage-3-recovery-manual-rescue.md) and [Stage 6](development-plans/stage-6-localization-accessibility.md); each plan's own scope, contracts, acceptance tests, and exit gate control that work. Stage 7 stays out of scope until both close.
